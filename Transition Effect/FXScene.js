...
function getMesh(material, needsAnimatedColor = false) {
    ...
    for (let i = 0; i < objCount; i++) {
        ...
        if (needsAnimatedColor) { mesh.setColorAt(i, color.setHSL(Math.random(), 1.0, 0.5)); } //random colours transition
    }
    return mesh;
}
...
export function getFXScene({ renderer, material, clearColor, needsAnimatedColor = false }) {
    ...
    const fbo = new THREE.WebGLRenderTarget(w, h); //frame buffer
